...
class SettingsController extends Controller
{
    ...
    public function index()
    {
        ...
        return Inertia::render('Admin/Settings/Index', [
            ...
            'vtpass' => [
                'api_key' => Setting::where('key', 'vtpass_api_key')->value('value'),
                'secret_key' => Setting::where('key', 'vtpass_secret_key')->value('value'),
                'api_url' => Setting::where('key', 'vtpass_api_url')->value('value'),
            ],
            'proSettings' => [
    ...
    private function getApiDetails()
    {
        ...
        $balanceResponse = $husmodataService->getBalance();
        $virtualAccountResponse = $husmodataService->getVirtualAccountDetails();
        ...
        return [
            ...
            'virtualAccountError' => !$virtualAccountResponse['success'] ? $virtualAccountResponse['message'] : null,
    ...
    public function update(Request $request)
    {
        ...
        $request->validate([
            ...
            'husmodata_api_key' => 'required|string',
            'husmodata_api_url' => 'required|url',
            // VTPass
            'vtpass_api_key' => 'nullable|string',
            'vtpass_secret_key' => 'nullable|string',
            'vtpass_api_url' => 'nullable|url',
            // XixaPay
        ...
        // API settings
        Setting::set('husmodata_api_key', $request->husmodata_api_key, 'api', false);
        Setting::set('husmodata_api_url', rtrim($request->husmodata_api_url, '/'), 'api', true);

        // VTPass settings
        if ($request->filled('vtpass_api_key')) {
            Setting::set('vtpass_api_key', $request->vtpass_api_key, 'api', false);
        }
        if ($request->filled('vtpass_secret_key')) {
            Setting::set('vtpass_secret_key', $request->vtpass_secret_key, 'api', false);
        }
        if ($request->filled('vtpass_api_url')) {
            Setting::set('vtpass_api_url', rtrim($request->vtpass_api_url, '/'), 'api', true);
        }
    ...
    /**
     * Sync data plans from the API.
     *
     * @return \Illuminate\Http\RedirectResponse
     */
    public function syncDataPlans()
    {
        // This would be implemented to fetch data plans from the Husmodata API
        // and update the local database

        return redirect()->route('admin.settings')->with('success', 'Data plans synced successfully');
    }
    ...
    {
        $husmodataService = app(HusmodataService::class);
        $response = $husmodataService->getBalance();

        if ($response['success']) {
            return redirect()->route('admin.settings')->with('success', 'API connection successful. Balance: ₦' . ($response['data']['balance'] ?? 'N/A'));
        }

        return redirect()->route('admin.settings')->with('error', 'API connection failed: ' . ($response['message'] ?? 'Unknown error'));
    }

    /**
     * Test VTPass API connection.
     */
    public function testVtpassConnection()
    {
        try {
            $vt = app(VtpassService::class);
            // Hit a lightweight endpoint (variations for dstv)
            $res = $vt->getVariations('dstv');
            if ($res['success']) {
                return redirect()->route('admin.settings')->with('success', 'VTPass connection successful. Variations loaded: ' . count($res['data']));
            }
            return redirect()->route('admin.settings')->with('error', 'VTPass connection failed: ' . ($res['message'] ?? 'Unknown error'));
        } catch (\Exception $e) {
            return redirect()->route('admin.settings')->with('error', 'VTPass connection failed: ' . $e->getMessage());
        }
    }
