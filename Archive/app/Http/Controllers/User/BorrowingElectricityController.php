...
class BorrowingElectricityController extends Controller
{
    ...
    public function __construct(
        ...
        \App\Services\VtpassService $vtpassService,
    ...
    public function index()
    {
        ...
        $providers = ElectricityProvider::where('status', true)->get();
        ...
        if (!$eligibility->isEligible()) {
            if (!$hasActiveCard) {
                ...
                $remainingDays = max(0, 7 - ($eligibility->eligibility_criteria['account_age_days'] ?? 0));
                if ($remainingDays > 0) {
                    $eligibilityResponse['action'] = "Your account will be eligible in $remainingDays day(s). Keep transacting to build your credit history.";
                    $eligibilityResponse['action_type'] = 'wait';
    ...
    public function verifyMeter(Request $request)
    {
        ...
        $verify = $this->wazobiaService->verifyElectricityMeter(
            $request->meter_number,
            $request->meter_type
        );

        if (!$verify['success']) {
            $verify = $this->vtpassService->verifyCustomer(
                $provider->code,
                $request->meter_number,
                'meter'
            );
