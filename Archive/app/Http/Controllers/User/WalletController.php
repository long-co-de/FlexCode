...
class WalletController extends Controller
{
    ...
    public function fund(Request $request)
    {
        ...
        if ($paymentMethod->code === 'paystack') {
            ...
            if ($response['status']) {
                // Store checkout URL in wallet funding meta data
                $walletFunding->response_data = array_merge($walletFunding->response_data ?? [], [
                    'checkout_url' => $response['data']['data']['authorization_url'] ?? null,
                ]);
                $walletFunding->save();

                // Redirect to Monnify checkout page
                return back()->withErrors(['url' => $response['data']['data']['authorization_url']]);
            } else {
    ...
    public function verifyPayment(Request $request)
    {
        ...
        if ($response['success']) {
            ...
            if ($paymentStatus === 'successful') {
                ...
                $fee = ($walletFunding->amount * $chargePercentage) / 100;
                $walletFunding->fee = $fee;
                $walletFunding->response_data = json_decode($walletFunding->response_data, 1) ?? [];
                $walletFunding->response_data = array_merge($walletFunding->response_data ?? [], [
                    'completed_at' => now(),
