...
export default function CableProvidersIndex({ auth, cableProviders, filter }) {
    const [searchTerm, setSearchTerm] = useState(filter?.search || '');

    const { get } = useForm();

    // Normalize providers list to support both paginated and plain arrays
    const rows = Array.isArray(cableProviders?.data)
        ? cableProviders.data
        : (Array.isArray(cableProviders) ? cableProviders : []);

    const handleSearch = (e) => {
        e.preventDefault();
        get(route('admin.cable-providers', { search: searchTerm }));
    };

    return (
        <AdminLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl igg-800 leading-tight">Manage Cable Providers</h2>}
        >
            <Head title="Manage Cable Providers" />
                        ...
                        <div className="p-6 bg-base-100 border-b border-gray-200">
                            <div className="flex flex-col md:flex-row justify-between items-center mb-6">
                                <div className="flex items-center mb-4 md:mb-0">
                                    <h3 className="text-lg font-medium mr-4">Cable Providers</h3>
                                    <Link href={route('admin.cable-providers.create')} className="btn-primary">
                            ...
                            <div className="overflow-x-auto">
                                            ...
                                            rows.map((provider) => (
                                                <tr key={provider.id}>
                                                    ...
                                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                        <Link
                                                            href={route('admin.cable-providers.edit', provider.id)}
                                                            className="text-primary-600 hover:text-primary-900 mr-3"
                                                        >
                                                            <PencilIcon className="h-5 w-5 inline" />
                                                        </Link>
                                                        <Link
                                                            href={route('admin.cable-providers.destroy', provider.id)}
                                                            method="delete"
                                                            as="button"
                                ...
                                {/* Pagination */}
                                {Array.isArray(cableProviders?.links) && (
                                    <div className="px-4 py-3 flex items-center justify-between border-t border-gray-200 sm:px-6">
                                        <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                                            <div>
                                                <p className="text-sm igg-700">
                                                    Showing <span className="font-medium">{cableProviders.from}</span> to{' '}
                                                    <span className="font-medium">{cableProviders.to}</span> of{' '}
                                                    <span className="font-medium">{cableProviders.total}</span> results
                                                </p>
                                            </div>
                                            <div>
                                                <nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                                                    {cableProviders.links.map((link, i) => (
                                                        <Link
                                                            key={i}
                                                            href={link.url}
