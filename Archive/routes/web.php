...
Route::middleware(['auth', 'verified', \App\Http\Middleware\CheckActive::class])->group(function () {
    ...
    Route::middleware(['auth', 'verified'])->prefix('admin')->name('admin.')->group(function () {
        ...
        Route::middleware(['admin'])->group(function () {
            ...
            Route::get('/settings/sync-data-plans', [SettingsController::class, 'syncDataPlans'])->name('settings.sync-data-plans');
            ...
            Route::post('/settings/test-vtpass', [SettingsController::class, 'testVtpassConnection'])->name('settings.test-vtpass');

            // Wazobia Integration
            Route::get('/wazobia', [\App\Http\Controllers\Admin\WazobiaController::class, 'index'])->name('wazobia.index');
            Route::post('/wazobia/sync', [\App\Http\Controllers\Admin\WazobiaController::class, 'sync'])->name('wazobia.sync');
            Route::post('/wazobia/test-connection', [\App\Http\Controllers\Admin\WazobiaController::class, 'testConnection'])->name('wazobia.test-connection');
...
Route::middleware(['auth', 'verified'])->group(function () {
    ...
    Route::prefix('borrow')->group(function () {
        ...
        Route::post('/{borrowing}/disable-auto-deduction', [BorrowingController::class, 'disableAutoDeduction'])->name('borrow.disable-auto-deduction');
    });
});
...
Route::middleware(['auth', 'verified'])->group(function () {
    ...
    Route::prefix('borrow')->name('borrow.')->group(function () {
        ...
        Route::post('/{borrowing}/disable-auto-deduction', [BorrowingController::class, 'disableAutoDeduction'])->name('disable-auto-deduction');
...
// Utility Routes For Shared Hosting Environments
...
//  vtu:check-transactions             Check status of pending transactions
//   vtu:update-data-plans              Update data plans from Husmodata API and store in database
//   vtu:update-prices  

Route::get('/run-scheduled-tasks', function () {
    Artisan::call('vtu:check-transactions');
    Artisan::call('vtu:update-data-plans');
    Artisan::call('vtu:update-prices');
