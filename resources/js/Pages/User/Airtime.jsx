...
export default function Airtime({ auth, networks, beneficiaries = [] }) {
    ...
    const networkColors = {
        'MTN': 'bg-[#FFCC00] dark:bg-[#FFCC00]',
        'GLO': 'bg-[#2BB22A] dark:bg-[#2BB22A]',
        'AIRTEL': 'bg-[#FF0000] dark:bg-[#FF0000]',
        '9MOBILE': 'bg-[#005733] dark:bg-[#005733]'
    };
        ...
        <AppLayout
            ...
            header={
                <div className="flex items-center justify-between">
                    <div>
                        <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100">Buy Airtime</h2>
                        <p className="text-sm text-slate-500 dark:text-slate-400">Fast and reliable top-up</p>
                    </div>
                    <div className="flex items-center gap-3">
                        <Link href={route('transactions')} className="btn btn-ghost btn-sm gap-2 text-slate-600 dark:text-slate-300">
                            <FaHistory />
                            <span className="hidden sm:inline">History</span>
                        </Link>
                        <div className="bg-white dark:bg-slate-800 px-4 py-2 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 flex items-center gap-2">
                            <div className="w-8 h-8 rounded-lg bg-sky-50 dark:bg-sky-900/30 flex items-center justify-center">
                                <FaWallet className="text-sky-600 dark:text-sky-400 text-sm" />
                            </div>
                            <div>
                                <p className="text-[10px] text-slate-400 dark:text-slate-500 font-medium uppercase tracking-wider leading-none">Balance</p>
                                <p className="text-sm font-bold text-slate-700 dark:text-slate-100">₦{auth.user.wallet_balance.toLocaleString()}</p>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-6">
                                ...
                                <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 shadow-sm border border-slate-100 dark:border-slate-700">
                                    <h4 className="text-sm font-bold text-slate-800 dark:text-slate-100 mb-4 flex items-center gap-2">
                                        <FaTag className="text-sky-500 dark:text-sky-400" />                                        Select Airtime Type
                                    </h4>
                                        ...
                                        {airtimeTypes.map((type) => (
                                            <button
                                                ...
                                                className={`relative p-4 rounded-2xl border-2 transition-all flex flex-col items-center gap-2 group ${
                                                    ...
                                                    : 'border-slate-50 dark:border-slate-700 hover:border-slate-200 dark:hover:border-slate-600 bg-slate-50/50 dark:bg-slate-700/50'
                                                }`}
                                            >
                                                ...
                                                <span className={`text-xs font-bold ${airtimeType === type.id ? 'text-sky-700 dark:text-sky-300' : 'text-slate-600 dark:text-slate-300'}`}>
                                                    {type.name}
                                                </span>
                                                {airtimeType === type.id && (
                                                    <div className="absolute -top-2 -right-2 w-5 h-5 bg-sky-500 dark:bg-sky-400 rounded-full flex items-center justify-center shadow-lg border-2 border-white dark:border-slate-800">
                                ...
                                <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 shadow-sm border border-slate-100 dark:border-slate-700">
                                    <h4 className="text-sm font-bold text-slate-800 dark:text-slate-100 mb-4 flex items-center gap-2">
                                        <FaSimCard className="text-sky-500 dark:text-sky-400" />
                                        Select Network
                                    </h4>
                                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                                        {networks.map((network) => {
                                            const name = network.name.toUpperCase();
                                            const color = networkColors[name] || 'bg-slate-400 dark:bg-slate-600';
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    className={`relative group p-1 rounded-2xl transition-all ${
                                                        isActive ? 'ring-2 ring-sky-500 dark:ring-sky-400 ring-offset-2 dark:ring-offset-slate-800' : ''
                                                    }`}
                                                >
                                                    <div className={`h-24 rounded-xl flex flex-col items-center justify-center text-white overflow-hidden shadow-sm transition-transform group-hover:scale-[1.02] ${color}`}>
                                                        <span className="text-lg font-black tracking-tighter italic opacity-90 mb-1">{name}</span>
                                                        {hasDiscount && (
                                                            <div className="bg-white/20 dark:bg-black/20 backdrop-blur-md px-2 py-0.5 rounded-full flex items-center gap-1">
                                                    ...
                                                    {isActive && (
                                                        <div className="absolute top-0 right-0 w-6 h-6 bg-sky-500 dark:bg-sky-400 rounded-full flex items-center justify-center shadow-lg border-2 border-white dark:border-slate-800 translate-x-1/3 -translate-y-1/3">
                                ...
                                <div className="grid md:grid-cols-2 gap-6">
                                    <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 shadow-sm border border-slate-100 dark:border-slate-700">
                                        <h4 className="text-sm font-bold text-slate-800 dark:text-slate-100 mb-4 flex items-center gap-2">
                                            <FaPhone className="text-sky-500 dark:text-sky-400" />
                                            Recipient
                                        </h4>
                                        <div className="space-y-4">
                                            <div>
                                                <div className="relative">
                                                    <TextInput
                                                        ...
                                                        className="w-full pl-12 rounded-2xl border-slate-100 dark:border-slate-700 focus:border-sky-500 dark:focus:border-sky-400 focus:ring-sky-200 dark:focus:ring-sky-900 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100"
                                            ...
                                            <div className="flex items-center gap-2">
                                                <button 
                                                    ...
                                                    className={`w-10 h-6 rounded-full transition-all relative ${saveAsBeneficiary ? 'bg-sky-500 dark:bg-sky-400' : 'bg-slate-200 dark:bg-slate-700'}`}
                                                >
                                                    <div className={`absolute top-1 w-4 h-4 rounded-full bg-white dark:bg-slate-100 transition-all ${saveAsBeneficiary ? 'left-5' : 'left-1'}`}></div>
                                                </button>
                                                <span className="text-xs font-medium text-slate-600 dark:text-slate-400">Save as beneficiary</span>
                                            </div>
                                            ...
                                            {saveAsBeneficiary && (
                                                <TextInput
                                                    ...
                                                    className="w-full rounded-2xl border-slate-100 dark:border-slate-700 focus:border-sky-500 dark:focus:border-sky-400 focus:ring-sky-200 dark:focus:ring-sky-900 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 text-sm"
                                        ...
                                        <div className="space-y-4">
                                            <div className="relative">
                                                <TextInput
                                                    ...
                                                    className="w-full pl-12 rounded-2xl border-slate-100 dark:border-slate-700 focus:border-sky-500 dark:focus:border-sky-400 focus:ring-sky-200 dark:focus:ring-sky-900 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 text-lg font-bold"
                                                    placeholder="0.00"
                                                />
                                                <div className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500 font-bold">₦</div>
                                            </div>
                                            <div className="grid grid-cols-4 gap-2">
                                                {[100, 200, 500, 1000].map(amt => (
                                                    <button
                                                        ...
                                                        className="py-2 rounded-xl border border-slate-100 dark:border-slate-700 text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors"
                                                    >
                                                        ₦{amt}
                                                    </button>
                                                ))}
                                            ...
                                            {data.use_bnpl && (
                                                <div className="mt-6 space-y-4 animate-in fade-in slide-in-from-top-4">
                                                    <h4 className="text-sm font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
                                                        <FaBolt className="text-sky-500 dark:text-sky-400" />
                                                        Repayment Duration
                                                    </h4>
                                                    <div className="grid grid-cols-2 gap-4">
                                                        ...
                                                        >
                                                            <span className={`text-sm font-bold ${data.duration === 3 ? 'text-sky-700 dark:text-sky-300' : 'text-slate-600 dark:text-slate-300'}`}>3 Days</span>
                                                            <span className={`text-[10px] font-bold ${data.duration === 3 ? 'text-sky-500 dark:text-sky-400' : 'text-slate-400 dark:text-slate-500'}`}>10% Interest</span>
                                                        </button>
                                                        <button
                                                            type="button"
                                                        ...
                                                        >
                                                            <span className={`text-sm font-bold ${data.duration === 7 ? 'text-sky-700 dark:text-sky-300' : 'text-slate-600 dark:text-slate-300'}`}>7 Days</span>
                                                            <span className={`text-[10px] font-bold ${data.duration === 7 ? 'text-sky-500 dark:text-sky-400' : 'text-slate-400 dark:text-slate-500'}`}>13% Interest</span>
                                                        </button>
                                                    </div>

                                                    {data.amount && (
                                                        <div className="bg-slate-900 dark:bg-slate-950 rounded-2xl p-4 text-white space-y-2 mt-4">
                            ...
                            <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 shadow-sm border border-slate-100 dark:border-slate-700 sticky top-8">
                                ...
                                <div className="space-y-4 mb-8">
                                    <div className="flex justify-between items-center py-3 border-b border-slate-50 dark:border-slate-700/50">
                                        ...
                                        <span className="text-sm font-bold text-slate-700 dark:text-slate-100">Airtime {airtimeType}</span>
                                    </div>
                                    <div className="flex justify-between items-center py-3 border-b border-slate-50 dark:border-slate-700/50">
                                        <span className="text-sm text-slate-500 dark:text-slate-400">Network</span>
                                        <span className="text-sm font-bold text-slate-700 dark:text-slate-100">{selectedNetwork?.name || '---'}</span>
                                    </div>
                                    <div className="flex justify-between items-center py-3 border-b border-slate-50 dark:border-slate-700/50">
                                        <span className="text-sm text-slate-500 dark:text-slate-400">Phone</span>
                                        <span className="text-sm font-bold text-slate-700 dark:text-slate-100">{data.phone_number || '---'}</span>
                                    </div>
                                    <div className="flex justify-between items-center py-3 border-b border-slate-50 dark:border-slate-700/50">
                                        <span className="text-sm text-slate-500 dark:text-slate-400">Amount</span>
                                        <span className="text-sm font-bold text-slate-700 dark:text-slate-100">₦{(parseFloat(data.amount) || 0).toLocaleString()}</span>
                                    </div>
                                    {discount > 0 && (
                                        <div className="flex justify-between items-center py-3 border-b border-slate-50 dark:border-slate-700/50">
                                            <span className="text-sm text-slate-500 dark:text-slate-400">Discount ({discount}%)</span>
                                            <span className="text-sm font-bold text-emerald-600 dark:text-emerald-400">-₦{(parseFloat(data.amount) * discount / 100 || 0).toLocaleString()}</span>
                                        </div>
                                    )}
                                </div>

                                <div className="bg-slate-900 dark:bg-slate-950 rounded-2xl p-5 mb-6 text-white text-center">
                                ...
                                <button
                                    ...
                                    className="w-full btn bg-sky-500 dark:bg-sky-600 hover:bg-sky-600 dark:hover:bg-sky-500 border-none text-white rounded-2xl h-14 font-bold text-lg shadow-lg shadow-sky-200 dark:shadow-sky-900/30 transition-all active:scale-[0.98] disabled:bg-slate-100 dark:disabled:bg-slate-700 disabled:text-slate-400 dark:disabled:text-slate-500 disabled:shadow-none"
                                >
                                    {processing ? (
                                ...
                                <div className="mt-6 flex items-center justify-center gap-4 text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-widest">
                                    <div className="flex items-center gap-1">
                                        <FaShieldAlt className="text-sky-500 dark:text-sky-400" />
                                        Secure
                                    </div>
                                    <div className="flex items-center gap-1">
                                        <FaBolt className="text-amber-500 dark:text-amber-400" />
                                        Instant
                                    </div>
                                </div>
                            </div>
                            ...
                            {beneficiaries.length > 0 && (
                                <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 shadow-sm border border-slate-100 dark:border-slate-700">
                                    <div className="flex items-center justify-between mb-4">
                                        <h4 className="text-sm font-bold text-slate-800 dark:text-slate-100">Recent</h4>
                                        <Link href={route('beneficiaries.index')} className="text-[10px] font-black text-sky-500 dark:text-sky-400 uppercase">View All</Link>
                                    </div>
                                        ...
                                        {beneficiaries.slice(0, 4).map((b) => (
                                            ...
                                            >
                                                ...
                                                <div className="flex-1">
                                                    <p className="text-xs font-bold text-slate-800 dark:text-slate-100 leading-none mb-1">{b.name}</p>
                                                    <p className="text-[10px] text-slate-400 dark:text-slate-500">{b.phone_number}</p>
                                                </div>
                                                <div className="w-6 h-6 rounded-full bg-sky-50 dark:bg-sky-900/30 flex items-center justify-center">
                                                    <FaArrowRight className="text-[10px] text-sky-500 dark:text-sky-400" />
                                                </div>
                                            </button>
                                        ))}
                                    </div>
                ...
                <div className="p-8">
                    <div className="text-center mb-8">
                        <div className="w-16 h-16 bg-sky-50 dark:bg-sky-900/30 rounded-2xl flex items-center justify-center mx-auto mb-4">
                            <FaShieldAlt className="text-3xl text-sky-500 dark:text-sky-400" />
                        </div>
                        <h3 className="text-xl font-bold text-slate-800 dark:text-slate-100">Transaction PIN</h3>
                        <p className="text-sm text-slate-500 dark:text-slate-400">Confirm your purchase of ₦{parseFloat(amountToPay).toLocaleString()}</p>
                    </div>
                    ...
                    <div className="flex justify-center gap-3 mb-8">
                        {pin.map((digit, index) => (
                            <input
                                ...
                                className="w-14 h-14 text-center text-2xl font-black border-2 border-slate-100 dark:border-slate-700 rounded-2xl focus:border-sky-500 dark:focus:border-sky-400 focus:ring-4 focus:ring-sky-100 dark:focus:ring-sky-900 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 transition-all"
                                autoFocus={index === 0}
                            />
                        ))}
                    </div>
                    ...
                    {pinError && (
                        <div className="bg-rose-50 dark:bg-rose-900/30 text-rose-600 dark:text-rose-400 p-3 rounded-xl text-xs font-bold text-center mb-6">
                            {pinError}
                        </div>
                    )}
                    ...
                    <div className="flex gap-3">
                        <button
                            ...
                            className="flex-1 h-12 rounded-xl text-sm font-bold text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-all"
                        >
                            Cancel
                        </button>
                        <button
                            ...
                            className="flex-1 h-12 bg-sky-500 dark:bg-sky-600 text-white rounded-xl text-sm font-bold shadow-lg shadow-sky-100 dark:shadow-sky-900/30 transition-all active:scale-[0.98] disabled:opacity-50"
                        >
                                ...
                                >
                                    ...
                                    <div className="relative z-10 space-y-3">
                                        <div className="w-12 h-12 sm:w-14 sm:h-14 bg-emerald-50 dark:bg-emerald-900/30 group-hover:bg-emerald-100 dark:group-hover:bg-emerald-900/50 rounded-2xl flex items-center justify-center transition-colors">
                                            <FaWallet className="w-6 h-6 sm:w-7 sm:h-7 text-emerald-600 dark:text-emerald-400" />
                                        </div>
                                        <div>
                                ...
                                >
                                    ...
                                    <div className="relative z-10 space-y-3">
                                        <div className="w-12 h-12 sm:w-14 sm:h-14 bg-sky-50 dark:bg-sky-900/30 group-hover:bg-sky-100 dark:group-hover:bg-sky-900/50 rounded-2xl flex items-center justify-center transition-colors">
                                            <FaBolt className="w-6 h-6 sm:w-7 sm:h-7 text-sky-600 dark:text-sky-400" />
                                        </div>
                                        <div>
